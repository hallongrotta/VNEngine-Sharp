...
namespace VNEngine
{
    ...
    public class Scene
    {
        ...
        public Scene copy()
        {
            string stractors = JsonSerializer.Serialize(this.actors);
            string strprops = JsonSerializer.Serialize(this.props);
            string strcams = JsonSerializer.Serialize(this.cams);

            Dictionary<string, ActorData> copied_actors = JsonSerializer.Deserialize<Dictionary<string, ActorData>>(stractors);
            Dictionary<string, PropData> copied_props = JsonSerializer.Deserialize<Dictionary<string, PropData>>(strprops);
            List<CamData> copied_cams = JsonSerializer.Deserialize<List<CamData>>(strcams);
        ...
        // Camera Manip
        public virtual int addCam(CamData cam_data)
        {
            this.cams.Add(cam_data);
            return this.cams.Count - 1;
        }

        public virtual void updateCam(int index, CamData cam_data)
        {
            this.cams[index] = cam_data;
        }

        public virtual int deleteCam(int index)
        {
