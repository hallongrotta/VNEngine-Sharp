...
namespace VNEngine
{
    static class UI
    {
        ...
        public static void sceneConsoleSkinWindowGUI(VNNeoController game, object windowid)
        {
            sceneConsoleWindowFunc(windowid);
        }

        public static void sceneConsoleSkinWindowGUIMin(VNNeoController game, object windowid)
        {
            minimizeWindowFunc(windowid);
        }

        public static void sceneConsoleWindowFunc(object windowid)
        {
            ...
            try
            {
                ...
                {
                    (string msg, (bool, bool) func_param, bool single_op) warning_params = ((string, (bool, bool), bool))Utils.sceneConsole.warning_param;
                ...
                else
                {
                    ...
                    if (Utils.sceneConsole.windowindex == 0)
                    {
                        ...
                        if (GUILayout.Button("Close console", GUILayout.Width(100)))
                        {
                            ...
                            Utils.sceneConsole.warning_param = (String.Format("Do you really want to close window? (<b><color=%s>Warning:</color> All current scenedata will be deleted</b>)", col), null, false);
                        }
                        GUILayout.EndHorizontal();
                        GUILayout.EndVertical();
                        GUI.DragWindow();
        ...
        public static void sceneConsoleMinistates()
        {
            ...
            foreach (var el in mslist)
            {
                ...
                ar = Ministates.ministates_calc_prefix(el.Item1);
                ...
                foreach (var el in mslist)
                {
                    ...
                    if (ar[0] == prefix)
                    {
                        ...
                        if (GUILayout.Button(ar[1], GUILayout.Width(tableBtnW)))
                        {
                            try
                            {
                                Ministates.ministates_run_elem(Utils.sceneConsole.game, el.Item2);
                            }
                            catch (Exception e)
                            {
        ...
        public static void sceneConsoleLdSvUI()
        {
            ...
            if (GUILayout.Button("<color=#00ff00>Load</color> scene data", GUILayout.Height(btnBigHeight), GUILayout.Width(210)))
            {
                if (Utils.sceneConsole.block.Count > 0)
                {
                    ...
                    Utils.sceneConsole.warning_param = ("Do you wish to load scenedata from current scene? (Will overwrite console data)", null, false);
                }
                else
                {
            ...
            if (GUILayout.Button("<color=#ff0000>Save</color> scene data", GUILayout.Height(btnBigHeight), GUILayout.Width(210)))
            {
                ...
                if (!(fld == null))
                {
                    ...
                    Utils.sceneConsole.warning_param = ("Scenedata exists. Overwrite?", fld, false);
                }
                else
                {
            ...
            if (GUILayout.Button("<color=#00ff00>Load</color> from file", GUILayout.Height(btnBigHeight), GUILayout.Width(210)))
            {
                ...
                {
                    Utils.sceneConsole.warning_action = Utils.sceneConsole.loadSceneData;
                    Utils.sceneConsole.warning_param = ("Do you wish to load scenedata from file? (Will overwrite console data)", (true, false), false);
                }
                else
                {
            ...
            if (GUILayout.Button("<color=#ff0000>Save</color> to file", GUILayout.Height(btnBigHeight), GUILayout.Width(210)))
            {
                ...
                if (!(fld == null))
                {
                    ...
                    Utils.sceneConsole.warning_param = ("Scenedata exists. Overwrite?", false, false);
                }
                else
                {
            ...
            if (GUILayout.Button("Load backup scene data\n(scene/external file)", GUILayout.Height(btnSmallHeight), GUILayout.Width(210)))
            {
                ...
                if (Utils.sceneConsole.block.Count > 0)
                {
                    ...
                    {
                        Utils.sceneConsole.warning_action = Utils.sceneConsole.loadSceneData;
                        Utils.sceneConsole.warning_param = ("Do you wish to load backup scenedata from scene? (Will overwrite console data)", (false, true), false);
                    }
                    else
                    {
                        Utils.sceneConsole.warning_action = Utils.sceneConsole.loadSceneData;
                        Utils.sceneConsole.warning_param = ("Do you wish to load backup scenedata from file? (Will overwrite console data)", (true, true), false);
                    }
                }
                else if (fld == null)
                {
            ...
            if (GUILayout.Button("Load auto-timer backup file", GUILayout.Height(btnSmallHeight), GUILayout.Width(210)))
            {
                ...
                if (Utils.sceneConsole.block.Count > 0)
                {
                    ...
                    Utils.sceneConsole.warning_param = ("Do you wish to load backup scenedata from file auto-saved by timer? (Will overwrite console data)", null, false);
                }
                else
                {
        ...
        public static void sceneConsoleEditUI()
        {
            ...
            if (Utils.sceneConsole.cur_index > -1)
            {
                ...
                if (GUILayout.Button("Del", GUILayout.Width(Utils.sceneConsole.camviewwidth * 0.3f)))
                {
                    if (Utils.sceneConsole.promptOnDelete)
                    {
                        ...
                        Utils.sceneConsole.warning_param = ("Delete selected cam?", "del", false);
                    }
                    else
                    {
            ...
            if (GUILayout.Button("Delete scene"))
            {
                if (Utils.sceneConsole.promptOnDelete == true)
                {
                    ...
                    Utils.sceneConsole.warning_param = ("Delete selected scene?", null, false);
                }
                else
                {
